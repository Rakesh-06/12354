...
// Replaced all querySelecotrs with getElementById and getElementsByClassName
...
var resizePizzas = function(size) {
  ...
  // Eliminated changePizzaSize function too and added the width changes in changeSliderLabel function (on reviewer suggestion)
  function changeSliderLabel(size) {
    ...
    for (var i = 0; i < pizzalen; i++) {
      randomPizzasClass[i].style.width = newWidth + "%";     // width of each pizza
    }
  }
...
// Appends all of the pizzas when the page loads
var pizzasDiv = document.getElementById('randomPizzas');      //Placed outide for-loop to avoid redundancy
for (var i = 2; i < 100; i++) {
...
function updatePositions() {
  ...
  for (var i = 0; i < itemslen; i++) {
    phase = Math.sin(scrollPhase + (i % 5));      //simplified
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var cols = 8;
  var s = 256;
  // (on reviewer's suggestion) pizza rows and no. of moving pizzas are claculated based on the screen height.
  var pizzaRows = window.screen.height / s;
  var noOfPizzas = pizzaRows * cols;
  // console.log(pizzaRows, noOfPizzas);
  var movingPizzasID = document.getElementById('movingPizzas1');    // moved out of loop and replaced with getElementById
  for (var i = 0; i < noOfPizzas; i++) {
    ...
    movingPizzasID.appendChild(elem);
  }
  updatePositions();
});
